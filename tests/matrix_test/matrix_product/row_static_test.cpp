...
BOOST_AUTO_TEST_CASE_TEMPLATE(matrix_product_matrix_row_static_test, Cases, CheckTypeCases) {
    ...
    auto A = MatrixRowStatic<T, row1, Backend>(column1, arrayA);
    ...
    auto typeCheckBA = std::is_same<decltype(BA), MatrixRowStatic<T, row1, Backend>>::value;
    ...
    auto C = MatrixRowStatic<T, row2, Backend>(column2, arrayC);
    ...
    auto typeCheckDC = std::is_same<decltype(DC), MatrixRowStatic<T, row2, Backend>>::value;
    ...
    auto E = MatrixRowStatic<T, rowE, Backend>(columnE, arrayE);
    ...
    auto typeCheckFE = std::is_same<decltype(FE), MatrixRowStatic<T, rowF, Backend>>::value;
    ...
    auto G = MatrixRowStatic<T, rowG, Backend>(columnG, arrayG);
    ...
    auto typeCheckHG = std::is_same<decltype(HG), MatrixRowStatic<T, rowH, Backend>>::value;
    ...
    auto I = MatrixRowStatic<T, rowI, Backend>(columnI, arrayI);
    ...
    auto typeCheckJI = std::is_same<decltype(JI), MatrixRowStatic<T, rowJ, Backend>>::value;
    ...
    auto K = MatrixRowStatic<T, rowK, Backend>(columnK, arrayK);
    ...
    auto KK = K.Dot(K);
    auto typeCheckKK = std::is_same<decltype(KK), MatrixRowStatic<T, rowK, Backend>>::value;
    ...
    auto N = MatrixRowStatic<T, 3, Backend>(3, nArray);
