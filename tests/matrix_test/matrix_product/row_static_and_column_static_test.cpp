...
BOOST_AUTO_TEST_CASE_TEMPLATE(matrix_row_static_and_matrix_column_static_test, T, MatrixProductCheckType) {
    ...
    auto A = MatrixRowStatic<T, row1>(column1, ArrayToVector(arrayA));
    ...
    auto C = MatrixRowStatic<T, row2>(column2, ArrayToVector(arrayC));
    ...
    auto E = MatrixRowStatic<T, rowE>(columnE, ArrayToVector(arrayE));
    ...
    auto G = MatrixRowStatic<T, rowG>(columnG, ArrayToVector(arrayG));
    ...
    auto I = MatrixRowStatic<T, rowI>(columnI, ArrayToVector(arrayI));
    ...
    auto mArray = std::array<T, 3 * 2>{2.4f, 5.4f, 7.1f, 3.1f, 2.4f, 4.8f};
    auto nArray = std::vector<T>{2.3f, 2.9f, 3.5f, 5.7f, 3.8f, 7.3f, 4.2f, 1.5f, 2.7f};
    auto M = MatrixRowStatic<T, 3>(2, ArrayToVector(mArray));
