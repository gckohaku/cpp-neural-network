...
BOOST_AUTO_TEST_CASE_TEMPLATE(matrix_product_matrix_static_and_matrix_dynamic_test, Cases, CheckTypeCases) {
    ...
    auto M = Matrix<T, 3, 2, Backend>(mArray);
    auto N = MatrixDynamic<T, Backend>(3, 3, nArray);

    BOOST_CHECK_THROW(M.Dot(N), std::domain_error);
    BOOST_CHECK_NO_THROW(N.Dot(M));

    auto O = MatrixDynamic<T, Backend>(3, 2, ArrayToVector<T>(mArray));
    auto P = Matrix<T, 3, 3, Backend>(VectorToArray<T, 9>(nArray));
