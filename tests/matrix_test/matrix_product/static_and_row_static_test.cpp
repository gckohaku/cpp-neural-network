...
BOOST_AUTO_TEST_CASE_TEMPLATE(matrix_product_matrix_static_and_matrix_row_static_test, Cases, CheckTypeCases) {
    ...
    auto A = Matrix<T, row1, column1, Backend>(arrayA);
    auto B = MatrixRowStatic<T, row1, Backend>(column1, arrayB);
    ...
    auto typeCheckAB = std::is_same<decltype(AB), MatrixRowStatic<T, row1, Backend>>::value;
    auto typeCheckBA = std::is_same<decltype(BA), Matrix<T, row1, column1, Backend>>::value;
    ...
    auto C = Matrix<T, row2, column2, Backend>(arrayC);
    auto D = MatrixRowStatic<T, row2, Backend>(column2, arrayD);
    ...
    auto typeCheckCD = std::is_same<decltype(CD), MatrixRowStatic<T, row2, Backend>>::value;
    auto typeCheckDC = std::is_same<decltype(DC), Matrix<T, row2, column2, Backend>>::value;
    ...
    auto E = Matrix<T, rowE, columnE, Backend>(arrayE);
    auto F = MatrixRowStatic<T, rowF, Backend>(columnF, arrayF);
    ...
    auto typeCheckEF = std::is_same<decltype(EF), MatrixRowStatic<T, rowE, Backend>>::value;
    auto typeCheckFE = std::is_same<decltype(FE), Matrix<T, rowF, columnE, Backend>>::value;
    ...
    auto G = Matrix<T, rowG, columnG, Backend>(arrayG);
    auto H = MatrixRowStatic<T, rowH, Backend>(columnH, arrayH);
    ...
    auto typeCheckHG = std::is_same<decltype(HG), Matrix<T, rowH, columnG, Backend>>::value;
    ...
    auto I = Matrix<T, rowI, columnI, Backend>(arrayI);
    auto J = MatrixRowStatic<T, rowJ, Backend>(columnJ, arrayJ);
    ...
    auto typeCheckJI = std::is_same<decltype(JI), Matrix<T, rowJ, columnI, Backend>>::value;
    BOOST_CHECK(typeCheckJI);
    CheckCloseEachVectorElement(JI.Elements().GetVector(), expectedJI, acceptableErrorJI);

    auto K = MatrixRowStatic<T, rowI, Backend>(columnI, std::vector<T>(arrayI.begin(), arrayI.end()));
    auto arrayL = std::array<T, elementSizeJ>();
    std::copy_n(arrayJ.begin(), elementSizeJ, arrayL.begin());
    auto L = Matrix<T, rowJ, columnJ, Backend>(arrayL);

    auto LK = L.Dot(K);

    auto typeCheckKL = std::is_same<decltype(LK), MatrixRowStatic<T, rowJ, Backend>>::value;
    ...
    auto M = MatrixRowStatic<T, 3, Backend>(2, mArray);
    auto N = Matrix<T, 3, 3, Backend>(nArray);
